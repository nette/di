...
Assert::exception(function () {
	...
	$builder->generateClasses();
}, 'Nette\InvalidStateException', 'Unused parameter $baz when implementing method Bad4::create(), did you mean $bar?');
...
Assert::exception(function () {
	...
	$builder->generateClasses();
}, 'Nette\InvalidStateException', 'Unused parameter $baz when implementing method Bad6::create().');
