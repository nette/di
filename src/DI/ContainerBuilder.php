...
class ContainerBuilder implements Definitions
{
	...
	{
		return $this->registerDefinition($name, $definition);
	}


	/**
	 * Registers a definition under a name (or an autogenerated one), attaches the notifier and journals it.
	 * @template TDef of Definition
	 * @param  TDef|null  $definition
	 * @return ($definition is null ? Definitions\ServiceDefinition : TDef)
	 */
	private function registerDefinition(?string $name, ?Definition $definition = null): Definition
	{
	...
	/** @deprecated Use add() with a ready-made Nette\DI\Definitions\LocatorDefinition */
	public function addLocatorDefinition(?string $name): Definitions\LocatorDefinition
	{
		return $this->registerDefinition($name, new Definitions\LocatorDefinition);
	}
	...
	/**
	 * Excludes classes and interfaces from autowiring: services are never wired by these types.
	 * @param  class-string[]  $types
	 */
	public function excludeFromAutowiring(array $types): static
	{
		$this->needsResolve = true;
	...
	{
		return $this->excludeFromAutowiring($types);
	}
	...
	/**
	 * A service name never contains a backslash (enforced at registration), so a backslashed
	 * positional argument is always a mistaken type - teach instead of silently failing.
	 */
	private static function checkServiceName(?string $name, string $method): void
	{
		if ($name !== null && str_contains($name, '\\')) {
			throw new Nette\InvalidArgumentException(sprintf(
				'Service name never contains a backslash; did you mean %s(type: %s)?',
				$method,
				$name,
			));
		}
